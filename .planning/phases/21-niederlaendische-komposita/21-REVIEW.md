...
fix_status:
  fixed_at: 2026-09-25
  resolved: [CR-01, WR-01, WR-02, IN-02, IN-03]
  partially_resolved: [IN-01]
  open: [IN-04, IN-05]
---
...
## Fix-Stand (25.09.2026)

| Befund | Stand | Commit(s) |
|--------|-------|-----------|
| CR-01 | behoben (nl-Modul, dazu deutscher Zwilling und zweites Netz in `expected_marks`) | 24c33fa, 105e4aa |
| WR-01 | behoben (Locks um beide Automaten-Caches, Barrier-Test mit vier Threads) | d0fa2fa |
| WR-02 | behoben (`enabled_handler` stellt die Rebuild-Frage, Task auf Modulebene) | f6ea447 |
| IN-01 | teilweise behoben: Logzeile nennt die Liste; Rückgabename bleibt bewusst `wordlist_hash` | 0960eaa |
| IN-02 | behoben (Kommentare nennen 24 bis 25 MB) | 8c1669e |
| IN-03 | behoben (Summary und Kommentare: fünf Fälle/Dokumente) | 555b46c |
| IN-04 | offen, dokumentiert | n/a |
| IN-05 | offen, dokumentiert | n/a |

Nach allen Fixes: volle Suite 2985 passed / 15 skipped, ruff check, ruff format --check, pyright (latest) und vulture grün. Baumhash-Ratsche je Commit nachgezogen, 57 Dateien.
...
**Status:** resolved (24c33fa, 105e4aa). `wordlist_nl.py` liest Artefakt und Digest-Datei mit `errors="replace"` (`_read_artifact`, neues `_read_digest`), ersetzte Bytes lassen den Digest-Vergleich scheitern und nehmen den Rebuild-Pfad. Der deutsche Zwilling `wordlist.py` im eigenen Commit gleich behandelt (gleiche Invariante M-18-06); `resources.expected_marks` fängt `UnicodeDecodeError` zusätzlich als zweites Netz. Rot-Beweis: die neuen Tests (`test_wordlist_nl.py`, `test_wordlist.py`, `test_read_side.py`) scheitern ohne die Fixes.
...
**Status:** resolved (d0fa2fa). `_GERMAN_LOCK` und `_DUTCH_LOCK` in analyzer.py umschließen Lookup und Bau; `dutch_chain_for` hält den Dutch-Lock auch über das Lesen des Artefakts. Neue Tests mit `threading.Barrier` und künstlich gestrecktem Bau (0,2 s) zeigen genau einen Bau bei vier gleichzeitigen Aufrufen, deutsch wie niederländisch; ohne Fix rot.
...
**Status:** resolved (f6ea447). Rebuild-Task und Stop-Event liegen auf Modulebene (`_REBUILDING`, `_STOP_REBUILD`); `_start_the_rebuild_if_due` bündelt die gemeinsamen Bedingungen (Lifespan aktiv, kein Lauf in Arbeit, Marken verlangen Rebuild) und wird vom Lifespan und von `enabled_handler` gerufen. Ein zweites Enable startet keinen zweiten Lauf; ein Fehler bei der Rebuild-Frage lässt das Enable nicht scheitern. Bestehende Lifespan-Tests unverändert; neu: Enable nach deaktiviertem Start (ohne Fix rot), zweites Enable, Enable bei übereinstimmenden Marken.
...
**Status:** partially resolved (0960eaa). `expected_marks` liest deutsche Liste und Dutch-Marke in zwei getrennten try-Blöcken, die Warnung nennt die betroffene Liste. Bewusst NICHT umgesetzt: die Konstante `UNPROVEN_WORDLIST_NL = "wordlist_hash_nl"` als Rückgabe von `version_drift`. `wordlist_hash_nl` steht in `MARKS_A_REBUILD_ANSWERS`, `_rebuild_is_due` würde dann einen Band-Run über eine nicht lesbare Liste starten. `reindexRequired` nennt deshalb weiter `wordlist_hash`; eine saubere Lösung braucht einen eigenen, nicht rebuild-auslösenden Namen (Owner-Entscheid, offen).
...
**Status:** resolved (8c1669e). Beide Kommentare nennen "roughly 24 to 25 MB" mit Verweis auf docs/performance.md (Nachtrag 25.09.2026), englisch wie aller Code-Kommentar.
...
**Status:** resolved (555b46c). Summary-Zeile zählt alle fünf Fälle auf, beide Kommentare sprechen von fünf Dokumenten. Keine Logik und keine Zusicherung geändert; die Phase-19-Kommentare über vier Dokumente (Zeilen 351, 818, 886) beschreiben den damaligen Stand und bleiben.
...
**Status:** open (dokumentiert). Kein Doku-/Kommentar-Fix, sondern eine Code-Änderung an der Dutch-Kette; nach Auftrag nicht im Fix-Umfang.
...
**Status:** open (dokumentiert). Verlangt einen neuen AST-Hash-Test; nach Auftrag nicht im Fix-Umfang.
