...
## Aus 22-13 (Nachanfahrt)

- **Die Auszugsroute kennt die Einwortregel nicht (V-22-01, zweite Route).** In der Nachanfahrt lieferte die erste kalte Einwortsuche 26 Treffer in 2.617 ms, aber im Kaltstart-Fenster stehen `/snippets innerMs 1510.5` und `/search innerMs 1505.4` gegen `ceilingMs 1500.0` (`rohdaten-nachanfahrt/m01-langsame-aufrufe-kaltstart-einwort.txt`). `api/snippets.py` baut die `SemanticSide` mit `may_load=query_may_load()` auch für eine einwortige Zeile und lädt bei Schalter 0 die Gewichte; die Warmsuche von 95c eine Sekunde danach endete am Deckel mit 0 Treffern. Kein Fix in Phase 22: Änderung am ausgelieferten Verhalten, gehört mit V-22-01 in den Backlog (Owner-Entscheid).
- **`00-typwechsel.sh vorpruefung` schreibt ohne `OUT` nach `rohdaten/`.** In der Nachanfahrt hat es vier Zeilen an die committete Datei der Hauptanfahrt angehängt; von Hand zurückgesetzt, bevor committet wurde. Das Werkzeug ist gefahren und prüfsummengeschützt und bleibt, wie es ist. Jede weitere Anfahrt setzt `OUT` für jeden Aufruf, auch für die Vorprüfung.
