...
| `test_admin_ui_contract.py` (Gate C) | The mechanically checkable prohibitions of the two design contracts, over six files since phase 9: `admin.php`, `admin.css` and `admin.js` of the administration page, and `search.php`, `search.css` and `search.js` of the result page. No markup built from a string in either script, in any of its four spellings, no unescaped printing in a template, no inline script, no literal colour in a stylesheet, no removed focus ring, no dash that is not a hyphen, no emoji, and none of the five Nextcloud APIs the contract retired. Two statements belong to the result page alone and are the mirror image of two that belong to the administration page: its script carries none of the four markers of a poll, and it never calls `preventDefault`, because the return contract of that page rests on the browser owning the navigation. The special tests of the administration page were deliberately not widened onto the new files. It says nothing about how either page looks; that is the human sight check. |
...
| `test_php_acl_boundary.py` | A second recheck loop in the PHP tree. The readability question is asked at exactly one place, `SearchService.php`, and every file that resolves a file id stands in a named register with its count, so a fourth resolution cannot appear quietly. It says nothing about whether the one recheck decides correctly; that is `SearchServiceTest.php` and the parity job. |
| `test_search_limits_lockstep.py` | A drift between `SEARCH_LIMIT_MAX` and `SEARCH_OFFSET_MAX` of the container and their two PHP mirrors. A mirror above the container turns a refused request into "the search does not answer", which is a false statement about the state of the server. It says nothing about whether either number is right; that argument stands at the constants themselves. |
...
## The gates of phase 13, and the boundary of each one

The filter and sort phase left three groups of gates behind, and they are easy
to mistake for one another, because every one of them can be read as "the filter
works". Not one of them says that on its own. The table lists them with what they
prevent and with what they deliberately do not prove, and the paragraph under it
is the division of labour between the three groups, written down because it
would otherwise have to be reconstructed from three plan summaries.

| Gate | What it prevents | What it does not prove |
|---|---|---|
| `test_search_fields_lockstep.py` (13-03) | A field that touches the query builder arriving in only one of the two request models. Both models refuse a field they do not declare, which is what keeps a caller from smuggling an identity past the signed header, and it has a second effect: a field only one side knows turns the excerpt call into a refusal, the refusal into nothing, and the page into a search that found hits and shows no excerpts, with nothing in either log saying why. The same gate holds the three sort names of the model against the keys of the table the engine looks the order up in, because that table answers an unknown name with the relevance branch and therefore never complains. | That a field is a good idea, that its bounds are right, or that the route does anything with it. Those arguments stand at the fields themselves. This gate insists on one thing: a decision about the wire is made once and arrives on both sides. |
| No script for the filter row, `test_the_page_script_knows_nothing_about_the_filter_row` (13-10) | The row being taken away from the browser. The script of the result page carries none of the names of the row, so every control stays an ordinary link on an address the server finished: the row works with script switched off, a middle click opens any of it in a tab, and the back button means what it always meant. | Behaviour. It reads names. A listener bound to a bare `a` element would carry none of these strings; what keeps that one out is the older prohibition on cancelling the default action of a click. |
| No second control channel, `test_the_filter_row_is_not_a_form_and_carries_no_select` (13-10) | A choice hidden behind a closed control, and a second form beside the search field. A dropdown needs a button or a listener, it hides the whole set of choices, and it cannot show which of them are in force. The count of forms is the anti vacuity clause of this gate. | What the one form sends. Which fields travel with it is decided in the controller and asserted on the PHP side. |
| No pressed link, `test_no_template_of_the_app_presses_a_link` (13-10) | A link marked up as a pressed button. That attribute belongs to a button role, a link with an address is a link, and a pressed link is nothing: a screen reader says something meaningless or says nothing, and the state of the chip is then carried by its colour alone. | Which element carried the attribute. The gate forbids the string in every template of the app and asks nothing else, because neither page has a control that could legitimately be pressed. |
| No counting oracle in the row, `scan_filter_row_for_a_counting_oracle` (13-10) | A number beside a chip, a dot, a greyed out chip or a plural sentence. All four are the same piece of information, and it would have to be worked out before the one place that knows what this user may open, so it would be an answer about other people's files. All chips therefore look alike, including the ones behind which there is nothing. | The controller. A count worked out in PHP and handed over under a harmless name would pass here and has to be caught where the chips are built. This gate holds the rendering. |
| Every control reachable, `test_every_control_of_the_filter_row_can_be_reached` (13-10) | A control too small for a thumb and a row that scrolls sideways. Each control is at least as tall as a clickable area of the theme and grows under a coarse pointer, and the containers of the row wrap instead of pushing part of the choice out of sight, where a keyboard cannot reach it at all. | A layout. It reads declarations. A rule that is present and overridden further down, or a container that wraps while its parent clips it, would pass; the sight check on a narrow screen stays a human one. |
| No style attribute in a template, inside `scan_template` (13-10) | A declaration no stylesheet of this app can be read out of, that no theme variable reaches and that the next dark mode forgets. It was added to the scanner both templates already run through rather than written as a gate of its own, because the rule is the same for both. | Anything about a stylesheet. The literal colours and the removed focus ring are the business of the stylesheet scanner. |
| Scenario 11 of `search-parity`, the type filter that takes nothing away (13-12) | A type filter losing a hit the user is allowed to see, or showing one the native search does not show him. The marker files of that job are all of one type, and the group the filter names carries that type, so the narrowed page has to answer the set of the unnarrowed dialog and the expected minimum stays where it was. | That a filter takes anything away at all. Under this filter nothing is meant to disappear, which is the whole reason the set can be compared symmetrically. |
| Scenario 12 of `search-parity`, the sort order that takes nothing away (13-12) | A sorted answer travelling a different road through the permission chain than an unsorted one. Under a sort mode the container leaves the fusion window unentered and answers out of a branch of its own, which calls the permission prefilter in a loop of its own; only a question under a sort mode reaches that second call. The set does not move, only the order inside it does. | Any order. Parity is a statement about sets of file ids and about nothing else. Which document stands first is the business of the backend tests and of FILT-02. |
| Scenario 13 of `search-parity`, the filter that really takes something away (13-12) | A narrowed page handing a user a file the native search does not hand him. It is the only one of the three that is not a symmetric comparison, because the filter reaches the page and neither of the two providers, so a narrowing that really removes something makes the two sides disagree by design. The removal is therefore measured directly, three hit rows without the filter and none with it, and the empty answer has to be the empty state of the page rather than its error block. The comparison beside it asks a user who may see none of those files the same narrowed question and is kept honest by a second one, same account and same minute, over files he really may see. | That the removed files were the right ones to remove. The scenario knows that the page emptied and that nobody was handed anything extra while it was empty. Which file belongs to which group is the business of the group table of the container and of the query tests. |

**The division of labour, and why no one of the three replaces another.** The
permission boundary is held by `test_php_acl_boundary.py` and
`test_php_trust_boundary.py`, and neither of them moved in this phase: the first
counts the call sites that resolve a file id and keeps a named register of them,
so a second recheck loop cannot appear quietly, and the second counts the
attributes of every route. Both are statements about absence, and absence is the
one thing a functional test cannot show. The parity scenarios are the other end
of the same sentence: they show the effect against a real Nextcloud, with a real
user, and they can only ever say something about the questions they were built
to ask. The surface gates say nothing about permissions at all; they hold the
promise that the row stays a row of links and that it never puts a number in
front of somebody about files that are not his. A reader who quotes any one of
the three for the claim of another has the wrong sentence.
...
### Three more, from phase 9

The twelve above stay twelve, because they are the specification of one audit
follow up and a list that grows with every phase can no longer be read against
the audit it came from. What phase 9 added is written here instead, in the same
shape and with the same column: the behaviour, and the file that holds it.

| No | The behaviour | Held by | Since |
|---|---|---|---|
| 13 | The recheck exists exactly once. `SearchService::run` is the one place that resolves a candidate through the folder of the asking user and asks `isReadable`, and both callers, the search provider of the dialog and the controller of the result page, hand their own ceilings into it and bring no decision of their own. | `SearchServiceTest.php`, and `test_php_acl_boundary.py` for the count of call sites | 09-03 |
| 14 | `Highlighter::segments` turns a text and its ranges into pieces with a mark, sorts the ranges itself, discards a range that reaches into a consumed one, ends where it starts or reaches past the end of the text, counts the offsets in characters rather than in bytes, and reassembles the input exactly. The template escapes every piece on its own, so no string carrying both text and markup ever exists. | `HighlighterTest.php` | 09-04 |
| 15 | `PageController::index` reads its own address defensively: a term beyond the ceiling is clamped and one that is not valid UTF-8 is no term at all, every page number outside the range is page one, and a cursor path whose length, order or first element does not check out falls back to page one of the same search, silently. The next and previous addresses are built from that checked path and never offered into nothing. | `PageControllerTest.php` | 09-04 |
