...
1. `ExAppService::filterHits` drops a hit whose `fileId`, `title` or `snippet`
   has the wrong type or is absent.
2. It drops a hit with a non positive `fileId` whose title is not the canary.
3. It drops a hit whose node cannot be resolved through the user's own folder.
4. It takes title and path from the resolved node and never from the answer.
5. It returns an empty list, not unchecked hits, when the user has no home
   folder.
6. `ExAppService::plainText` strips control characters, keeps the tab, caps the
   title at 255 and the snippet at 1000 characters, cuts on character boundaries
   and refuses invalid UTF-8.
7. `ExAppService::search` refuses an empty term without a round trip and clamps
   the limit into 1..100.
...
The other eight are unit test material.
...
that `OCP` is available and `IRootFolder`, `IUserManager`, `IAppManager` and
`LoggerInterface` can be mocked with `createMock`.
...
up red for reasons that have nothing to do with the code under test, and the nine
items above are the specification for whoever writes it with a PHP runtime at
hand. Until then the gap is here, in writing, and not in somebody's memory.
