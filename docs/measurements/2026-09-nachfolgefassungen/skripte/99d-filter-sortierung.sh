...
# docs/measurements/2026-09-v12-messung/skripte/99c-filter-sortierung.sh, und
# ihr Anlass ist der Befund L-04 der Abnahme vom 21.09.2026 (Auflage A1). Die
# Vorgaengerin ist am 21.09.2026 auf der bezahlten Box gefahren worden, ihre
# Rohdaten liegen neben ihr, und damit ist sie Teil des Belegs: sie wird nicht
# editiert. Der Waechter DRIVEN_V12_FASSUNGEN in
# backend/tests/test_measurement_scripts.py wird beim ersten geaenderten Byte
# drueben rot, und das ist Absicht.
#
# **DIESE FASSUNG IST NICHT GEFAHREN.** Neben ihr liegt keine Rohdatei, und es
# gibt keine Box mehr, auf der sich ihre Wirkung nachmessen liesse. Ihr
# Abnahmekriterium war Form, Pruefsumme und statische Analyse. Was sie wirklich
# bewirkt, misst erst die naechste Anfahrt. Wer hier Zahlen sucht, findet
# keine, und das ist kein Versehen.
#
# **Die eine Aenderung gegen 99c.** Das Passwort kommt jetzt auch aus der
# Passwortdatei. 99c kannte allein die Umgebungsvariable, deren NAMEN
# PASSWORT_ENV traegt; die uebrigen Werkzeuge desselben Laufverzeichnisses
# lesen daneben PWFILE, und zwar 95-spitze.sh, 96b-waechter.sh und
# 97-nebenlaeufigkeit.sh, alle drei mit derselben Vorgabe. 98c-sprachfaelle.sh
# fuehrt dieselbe Variable PWFILE, liest sie aber ausdruecklich nicht mehr,
# weil ihre Vorpruefung kein zweites Konto-Passwort braucht; der NAME kommt
# deshalb von dort, die LESEFORM von den drei Werkzeugen daneben. Ein Lauf, der
# die Datei hinterlegt und die Umgebung nicht gesetzt hatte, meldete sich bei
# 99c still als niemand an: die Seitenroute antwortet dann mit der Anmeldeseite,
# und die ist schnell. Das ist kein Sicherheitsdefekt, denn die Umgebung ist
# nach V14 die erlaubte Form und bleibt es; es ist ein Bruch der
# Einheitlichkeit, und genau so fuehrt ihn der Bericht.
#
# Die Reihenfolge ist Umgebung zuerst, Passwortdatei danach, nach dem Vorbild
# von 98b-sprachfaelle.sh. Steht am Ende kein Passwort da, endet dieses
# Werkzeug mit 2 und der Benutzung auf stderr, bevor die erste Anfrage laeuft;
# 2 ist die Zahl, die dieses Laufverzeichnis der fehlenden Pflichtangabe gibt
# (92b-wechsel.sh und ABBILD_DIGEST), und keine neue.
#
# **Was ausdruecklich gleich bleibt**, damit ein Bericht dieser Fassung neben
# einem Bericht der Vorgaengerin gelesen werden kann:
#
#   - alle Rueckgabewerte mit ihrer Bedeutung, 2, 29, 34 und 35, und keine Zahl
#     wird umgehaengt
#   - alle Fristen, Vorgaben und Zaehlungen, einschliesslich BEGRIFF,
#     FILTER_GRUPPE, SORTIERMODI, SEITEN und RUNDEN
#   - die Messreihenfolge, Block A vor Block B, beide in einem Zug und in
#     derselben Aufwaermung
#   - die Adressbildung ueber den GEZOGENEN Weiter-Link und nie ueber eine
#     gebaute Adresse, samt dem Abbruch, wenn der Link fehlt
#   - der Anmeldeweg selbst, ausschliesslich die Sitzung, und die Zeile, die
#     den Verzicht auf die zweite Haelfte protokolliert
#   - dass kein Passwort je in einer Kommandozeile, einer Fehlermeldung oder
#     einer Rohdatei steht
#
# **Der eigene Name.** Das Werkzeug nennt sich 99d, schreibt
# 99d-filter-sortierung.txt und meldet 99D-FILTER-SORTIERUNG-FERTIG. Das ist
# keine Aenderung in der Sache, sondern dieselbe Umbenennung, die 99c
# gegenueber 99b-runden.sh und 99-seitenroute.sh vorgenommen hat: eine Rohdatei
# nennt das Werkzeug, das sie geschrieben hat.
#
# Der Kopf der Vorgaengerin steht unveraendert darunter, weil er diese Fassung
# genauso beschreibt.
#
...
#
#   2  das Werkzeug wurde mit einem Argument gerufen, das es nicht gibt, ODER
#      das Passwort steht weder in der Umgebung noch in der Passwortdatei
#      (L-04)
...
benutzung() {
    cat >&2 <<'HINWEIS'
Benutzung: 99d-filter-sortierung.sh
...
ueber den NAMEN einer Umgebungsvariablen und nie ueber ein Argument; ist diese
Variable leer, wird die Datei gelesen, auf die PWFILE zeigt.
HINWEIS
}
...
# Die Passwortdatei des Lastkontos auf der Box, mit der Vorgabe, die
# 95-spitze.sh, 96b-waechter.sh und 97-nebenlaeufigkeit.sh desselben
# Laufverzeichnisses fuehren. Der Wert wird mit sudo gelesen und nie einem
# Befehl als Argument gegeben. Sie ist der zweite Weg und nicht der erste: die
# Umgebung bleibt die erlaubte Form nach V14.
...
mkdir -p "$OUT"
ZIEL="${ZIEL:-$OUT/99d-filter-sortierung.txt}"
WORK=$(mktemp -d)
chmod 700 "$WORK"
trap 'rm -rf "$WORK"' EXIT

# L-04, und dies ist die eine Aenderung dieser Fassung. Das Passwort kommt in
# eine Datei ohne abschliessenden Zeilenumbruch; curl liest den Feldwert
# daraus. Ein Zeilenumbruch reiste als %0A mit und waere Teil des Passworts.
#
# Zuerst die Umgebung, danach die Passwortdatei, nach dem Vorbild von
# 98b-sprachfaelle.sh. Der Wert wird dabei in KEINE Variable gelegt und in
# keine Kommandozeile: beide Quellen schreiben unmittelbar in dieses Feld,
# damit auch eine Zeile unter set -x nichts hergibt. Die Pruefung auf Existenz
# und Lesbarkeit ist die des Vorbilds: gelesen wird mit sudo, ein Fehlschlag
# laesst das Feld leer, und die Leere ist das Urteil.
PWFELD="$WORK/pwfeld"
: >"$PWFELD"
chmod 600 "$PWFELD"
...
if [ ! -s "$PWFELD" ]; then
    echo "99d-filter-sortierung: das Passwort steht weder in der Umgebungsvariablen," >&2
    echo "99d-filter-sortierung: deren Namen PASSWORT_ENV traegt, noch in der Datei, auf" >&2
    echo "99d-filter-sortierung: die PWFILE zeigt. Ohne Anmeldung antwortet die Seitenroute" >&2
    echo "99d-filter-sortierung: mit der Anmeldeseite, und die ist schnell: im Protokoll" >&2
    echo "99d-filter-sortierung: stuenden huebsche Zahlen fuer eine Messung, die nie" >&2
    echo "99d-filter-sortierung: stattgefunden hat (L-04)" >&2
    benutzung
    exit 2
fi
...
# der gefallen ist.
if [ -f "$WORK/schalter-unlesbar" ]; then
    echo "99d-filter-sortierung: die Stellung des Entladeschalters war nicht ablesbar" >&2
    echo "99d-filter-sortierung: ein Lauf ohne protokollierte Stellung gilt als unvollstaendig" >&2
    echo "99d-filter-sortierung: das ist Abschnitt 6.4 des Runbooks und keine Formalie" >&2
    exit 29
fi
if [ -f "$WORK/bestand-waechst" ]; then
    echo "99d-filter-sortierung: der Arbeitsvorrat war nicht leer, der Bestand wuchs noch" >&2
    echo "99d-filter-sortierung: die Endzahl des Volllaufs muss stehen, sonst misst die" >&2
    echo "99d-filter-sortierung: Sortierung zwei verschiedene Bestaende unter einer Zahl" >&2
    exit 34
fi
if [ -f "$WORK/sortierung-ohne-treffer" ]; then
    echo "99d-filter-sortierung: ein Sortierlauf hat keine Trefferzahl hergegeben" >&2
    echo "99d-filter-sortierung: Seite ohne Trefferblock, Anmeldung verloren oder HTTP ungleich 200" >&2
    echo "99d-filter-sortierung: eine Zeitnahme auf einer Fehlerseite ist keine Messung" >&2
    exit 34
fi
if [ -f "$WORK/stufe-ohne-zahlen" ]; then
    echo "99d-filter-sortierung: eine Stufe des Blaetterns lieferte keine Antwortzahlen" >&2
    echo "99d-filter-sortierung: eine Stufe ohne Zahlen ist unbrauchbar und wird nicht geschaetzt" >&2
    exit 35
fi
if [ -f "$WORK/cursor-verworfen" ]; then
    echo "99d-filter-sortierung: eine geholte Seite hat eine andere Seitenzahl gemeldet" >&2
    echo "99d-filter-sortierung: der Cursor ist verworfen worden, und dieser Rueckfall ist still" >&2
    echo "99d-filter-sortierung: ohne diesen Abbruch stuende er als schnelle Zahl im Protokoll" >&2
    exit 35
fi
if [ -f "$WORK/kennung-doppelt" ]; then
    echo "99d-filter-sortierung: zwei aufeinander folgende Seiten trugen dieselbe Datei-Kennung" >&2
    echo "99d-filter-sortierung: ein Blaettern, das eine Kennung zweimal ausliefert, ist ein" >&2
    echo "99d-filter-sortierung: Befund ueber die Seitenroute und keine Sortierzahl" >&2
    exit 35
fi
if [ -f "$WORK/kein-weiter-link" ]; then
    echo "99d-filter-sortierung: auf einer Seite stand kein Weiter-Link, obwohl noch Seiten" >&2
    echo "99d-filter-sortierung: erwartet wurden. Ersatzweise gebaut wird hier keine Adresse:" >&2
    echo "99d-filter-sortierung: eine gebaute Adresse ohne fp maesse den Rueckfall auf Seite 1" >&2
    exit 35
fi

echo "99D-FILTER-SORTIERUNG-FERTIG"
