...
# docs/measurements/2026-09-v12-messung/skripte/92b-wechsel.sh, und ihr Anlass
# ist der Befund L-03 der Abnahme vom 21.09.2026 (Auflage A1). Die
# Vorgaengerin ist am 20./21.09.2026 auf der bezahlten Box gefahren worden,
# ihre Rohdaten liegen neben ihr, und damit ist sie Teil des Belegs: sie wird
# nicht editiert. Der Waechter DRIVEN_V12_FASSUNGEN in
...
#
# **Die eine Aenderung gegen 92b.** Die Phase-B-Pipeline prueft den
# Rueckgabewert des occ-Aufrufs, der die ExApp registriert. In 92b stand dieser
# Aufruf in dem Block, dessen Ausgabe ueber tee in die Rohdatei laeuft. Der
# Rueckgabewert einer Pipeline gehoert zu ihrem letzten Befehl, hier also zu
# tee, und sh kennt kein pipefail. Ein gescheiterter Aufruf verliess deshalb
# nur die Subshell, die Verweigerungen unterhalb der Pipeline fanden keine
# Arbeitsdatei vor, und das Werkzeug endete mit 0. Genau das ist im ersten Lauf
# der Anfahrt geschehen: Rueckgabewert 0, ohne dass die Registrierung
# stattgefunden hatte. Der Aufruf schreibt jetzt in eine eigene Datei unter
# WORK, sein Rueckgabewert wird unmittelbar danach geprueft und in einer
# Arbeitsdatei vermerkt, und erst danach laeuft seine Ausgabe durch den Filter
# in die Rohdatei. Diese Datei traegt #!/bin/sh und behaelt ihn; pipefail ist
# kein Ausweg, der hier offenstuende.
#
...
#     Zahl wird umgehaengt. Die gescheiterte Registrierung bekommt den zweiten
#     Fall der 36, weil sie dasselbe bedeutet wie ein Container auf einer
#     fremden Abbildkennung: gemessen wuerde ein Stand, den der Baumhash nicht
#     belegt hat.
...
#     der Pflichtangabe ohne Vorgabewert
#   - die Zaehlung der laufenden Nextcloud-Instanzen, zweimal, in Phase A und
#     unmittelbar ueber dem Schalter --rm-data in Phase B
#   - die Reihenfolge der Phasen, der Ort der Verweigerungen unterhalb der
#     Pipeline und die Namen aller Zeilen, die dieses Werkzeug schreibt
#
# **Der eigene Name.** Das Werkzeug nennt sich 92c, schreibt 92c-wechsel.txt
# und 92c-info-box.xml und meldet 92C-WECHSEL-FERTIG. Das ist keine Aenderung
# in der Sache, sondern dieselbe Umbenennung, die 92b gegenueber
# 92-wechsel.sh vorgenommen hat: eine Rohdatei nennt das Werkzeug, das sie
# geschrieben hat.
#
# **Die zwei Baumhash-Werkzeuge liegen nicht neben dieser Datei.** Sie sind
# gefahrene Fassungen und bleiben im Laufverzeichnis der Anfahrt; die Variable
# WERKZEUGE unten zeigt dorthin. Ohne sie suchte diese Fassung die beiden
# Dateien in ihrem eigenen Verzeichnis und braeche mit 36 ab, obwohl nichts
# fehlte als ein Pfad.
#
# Der Kopf der Vorgaengerin steht unveraendert darunter, weil er diese Fassung
# genauso beschreibt. Die vier Unterschiede, die er nennt, gelten gegen
# docs/measurements/2026-09-vergleichsmessung-m7g/skripte/92-wechsel.sh und
# nicht gegen 92b.
#
...
# als Rohdatei $OUT/92c-info-box.xml liegen, damit der Bericht zeigen kann,
...
benutzung() {
    cat >&2 <<'HINWEIS'
Benutzung: ABBILD_DIGEST="sha256:<64 Hexziffern>" ./92c-wechsel.sh
...
mkdir -p "$OUT"
ZIEL="${ZIEL:-$OUT/92c-wechsel.txt}"
BAUMHASH="$OUT/40b-baumhash.txt"
INFO_KOPIE="$OUT/92c-info-box.xml"
WORK=$(mktemp -d)
chmod 700 "$WORK"
...
# Befehl der Phase B fallen muss und nicht danach.
{
    ...
    sed "s|<image-tag>[^<]*</image-tag>|<image-tag>$IMAGE_TAG</image-tag>|" \
        "$REPO/backend/appinfo/info.xml" >"$WORK/92c-info-box.xml"
    grep -E '<registry>|<image>|<image-tag>' "$WORK/92c-info-box.xml"
    cp "$WORK/92c-info-box.xml" "$INFO_KOPIE"
...
if [ -f "$WORK/zweite-instanz" ]; then
    echo "92c-wechsel: die Zaehlung meldet $(cat "$WORK/instanzen") statt genau einer Nextcloud" >&2
    echo "92c-wechsel: --rm-data traefe das Volumen jeder laufenden Instanz, weil der" >&2
    echo "92c-wechsel: Volumenname einer ExApp allein aus ihrer App-Kennung folgt (07.09.2026)" >&2
    exit 37
fi
if [ -s "$WORK/status.txt" ]; then
    echo "92c-wechsel: backend/appinfo/info.xml ist im Arbeitsbaum nicht sauber" >&2
    echo "92c-wechsel: ein Baumhash gegen einen veraenderten Arbeitsbaum belegt nichts," >&2
    echo "92c-wechsel: und der gemessene Stand waere nicht der Stand des Repositoriums" >&2
    exit 38
fi
if [ -f "$WORK/baumhash-fehlt" ]; then
    echo "92c-wechsel: der Baumhashbeweis hat nicht drei Hashes und ein ja hergegeben" >&2
    echo "92c-wechsel: Erfolgskriterium 1 waere nicht belegbar, und jede Zahl unterhalb" >&2
    echo "92c-wechsel: dieser Zeile gehoerte zu einem Zustand, den niemand benennen kann" >&2
    exit 36
fi
...
# Phase B, und ab hier wird die Box veraendert.
{
    ...
    if [ ! -f "$WORK/zweite-instanz-b" ]; then
        ...
        sudo docker cp "$INFO_KOPIE" "$NEXTCLOUD:/tmp/92c-info-box.xml"
        sudo docker exec "$NEXTCLOUD" chown 33:33 /tmp/92c-info-box.xml
        date -u +'register-start %Y-%m-%dT%H:%M:%SZ'
        # L-03, und dies ist die eine Aenderung dieser Fassung. Der Aufruf
...
if [ -f "$WORK/zweite-instanz-b" ]; then
    echo "92c-wechsel: die Zaehlung unmittelbar vor --rm-data meldet $(cat "$WORK/instanzen-b")" >&2
    echo "92c-wechsel: statt genau einer Nextcloud. Der Schalter ist nicht gefahren worden," >&2
    echo "92c-wechsel: und bis zur Klaerung wird nichts weiter getan (07.09.2026)" >&2
    exit 37
fi
if [ -f "$WORK/registrierung-fehlt" ]; then
    echo "92c-wechsel: der occ-Aufruf der Registrierung ist gescheitert (L-03)" >&2
    echo "92c-wechsel: die ExApp laeuft nicht auf dem geprueften Abbild, weil sie gar" >&2
    echo "92c-wechsel: nicht registriert worden ist. Das ist derselbe unbelegte Stand" >&2
    echo "92c-wechsel: wie ein Container auf einer fremden Abbildkennung, und er traegt" >&2
    echo "92c-wechsel: deshalb dieselbe Zahl" >&2
    exit 36
fi
if [ -f "$WORK/grenze-fehlt" ]; then
    echo "92c-wechsel: die cgroup meldet nicht $ERWARTETE_GRENZE/$ERWARTETER_SWAP in memory.max/memory.swap.max" >&2
    echo "92c-wechsel: ein Lauf, der gegen 4 GB misst, waehrend 2 GiB gemeint sind," >&2
    echo "92c-wechsel: misst eine andere Maschine als v1.1 und keine Abweichung" >&2
    exit 39
fi
if [ -f "$WORK/abbild-fremd" ]; then
    echo "92c-wechsel: der Container laeuft auf einer anderen Abbildkennung als der" >&2
    echo "92c-wechsel: geprueften. Der Baumhash hat ein Abbild belegt, gemessen wuerde" >&2
    echo "92c-wechsel: ein anderes, und das ist derselbe unbelegte Stand wie ein" >&2
    echo "92c-wechsel: fehlender Baumhash" >&2
    exit 36
fi
