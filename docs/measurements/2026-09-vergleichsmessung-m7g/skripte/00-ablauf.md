...
| 7 | `96-volllauf.sh` mit Wächter (Plan 10-04) | Plan 10-04 | Kriterium 1 und 2: der Volllauf, rund 19 Stunden, abgesetzt. Sampler daneben, Wächter mit Rundendeckel, `00-FERTIG` als Vertrag. Die Gesamtspitze und die Laufzeit entstehen hier und nirgends sonst. |
...
| 9 | `95-spitze.sh nachher` | `95-spitze-nachher.txt`, `95-nachher-stufe-<n>.json` | DI-07-02: der Kaltstart auf **vollem** Vektorbestand, also der schlechtere der beiden Fälle. Setzt einen bewussten `docker restart` des Backendcontainers voraus. **Die Reihenfolge ist zwingend: der OOM-Beweis wird VOR dem Neustart erhoben**, weil ein Neustart `memory.peak` und `memory.events` zurücksetzt. |
