...
| `Not readable for the users asked` | Für die gefragten Nutzer nicht lesbar | Não legível para os utilizadores consultados | Não legível para os usuários consultados |
| `The file is still there. Check the advanced permissions of the Team Folder: Findling reads a file only as a user who may open it and asks the first 20 of its users in alphabetical order.` | Die Datei ist noch vorhanden. Die erweiterten Berechtigungen des Team Folders prüfen: Findling liest eine Datei nur als Nutzer, der sie öffnen darf, und fragt die ersten 20 ihrer Nutzer in alphabetischer Reihenfolge. | O ficheiro continua a existir. Verifique as permissões avançadas da Team Folder: o Findling só lê um ficheiro em nome de um utilizador que o possa abrir e consulta os primeiros 20 dos seus utilizadores por ordem alfabética. | O arquivo continua existindo. Verifique as permissões avançadas da Team Folder: o Findling só lê um arquivo em nome de um usuário que possa abri-lo e consulta os primeiros 20 dos seus usuários em ordem alfabética. |
| `The user named in front of the path may not open this file. The diagnosis is about the file itself.` | Der vor dem Pfad genannte Nutzer darf diese Datei nicht öffnen. Die Diagnose gilt der Datei selbst. | O utilizador indicado antes do caminho não pode abrir este ficheiro. O diagnóstico refere-se ao próprio ficheiro. | O usuário indicado antes do caminho não pode abrir este arquivo. O diagnóstico se refere ao próprio arquivo. |
