...
| `Not readable for any user` | Für keinen Nutzer lesbar | Não legível por nenhum utilizador | Não legível por nenhum usuário |
| `Check the advanced permissions of the Team Folder. Findling only reads a file as a user who may open it.` | Die erweiterten Berechtigungen des Team Folders prüfen. Findling liest eine Datei nur als Nutzer, der sie öffnen darf. | Verifique as permissões avançadas da Team Folder. O Findling só lê um ficheiro em nome de um utilizador que o possa abrir. | Verifique as permissões avançadas da Team Folder. O Findling só lê um arquivo em nome de um usuário que possa abri-lo. |
