...
def _read_artifact(target: Path) -> list[str]:
    ...
    ``errors="replace"`` and never the strict read, which is the fail closed
    check below kept intact rather than loosened (review finding CR-01 of phase
    21, which named this line as the pre-existing twin of the Dutch one). A byte
    that is not UTF-8 used to raise a UnicodeDecodeError in front of the digest
    comparison, nothing on the way up to the lifespan catches that class, and the
    container did not start (the invariant of audit finding M-18-06). Replaced
    bytes change the content, so the comparison fails and the recipe runs again,
    exactly as for an edited file.
    """
    return target.read_text(encoding=ENCODING, errors="replace").split()
