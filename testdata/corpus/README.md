...
Seven small files that exist for one purpose: the CI job `readonly-gate` copies
...
format, an image, and two files that are broken on purpose.

Total size is under 4 KB, so the repository carries no binary weight.
...
| `03-document.docx` | Minimal but valid OOXML package, two paragraphs, one of them with German special characters | ZIP based office formats and their part relationships |
