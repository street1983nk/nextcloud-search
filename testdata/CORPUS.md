...
**A virus scanner cannot tell those six from the real thing, and it should not
have to.** A declared decompression bomb, a PDF with a JavaScript open action and
a PDF with an embedded file are exactly what a scanner is built to catch; every
one of them is generated by `scripts/dev/build_corpus.py`, carries no payload and
does nothing when opened, but a scanner reads structure and not intent. On a
Windows working copy the usual result is a quarantined file and a checkout that
looks broken rather than a message that explains itself, and the answer is an
exception for `testdata/corpus` and not a smaller corpus: the whole point of job
four is that these shapes travel the road a user document travels. Named here
because a contributor meets it before any documentation, security audit of plan
06.1-17, L3.
