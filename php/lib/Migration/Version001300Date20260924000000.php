...
/**
 * The minor step from 1.2.0 to 1.3.0, and the recorded version it invalidates.
 *
 ...
 * where it was. An instance going from 1.2.0 to 1.3.0 compares 1.3 against a
 * 1.2 that describes a container from before the update, finds a drift that
 ...
 * container as 1.1.0. The migration of 21.09.2026,
 * Version001200Date20260921000000, is what closed that for the previous minor,
 ...
 *
 * The rebuild of the index that this minor brings with it is deliberately not
 * here either. 1.3.0 writes its index with a text analysis 1.2.0 did not have,
 * so an index that already exists has to be walked once, and that walk takes
 * one to three hours on the hardware this app is built for. A run of that
 * length inside occ upgrade turns an app update into an outage: the instance
 * stays in maintenance mode for the whole of it, and the admin who clicked
 * update on a search app gets a Nextcloud that is down for the afternoon. So
 * the rebuild runs in the container, as a lifespan task, after the update is
 * over and the instance is back, and the search keeps answering out of the
 * index that is already there while it runs. This file stays what it is: ten
