 ...
 * 1.2.0 to 1.3.0, because that number was an answer about a container from
 ...
 *
 * The cases are the four of Version001200Date20260921000000Test plus two this
...
final class Version001300Date20260924000000Test extends TestCase {
	...
	public function testARecordedVersionIsDropped(): void {
		...
		$appConfig->method('getValueString')->willReturn('1.2.0');
		$appConfig->expects(self::once())
			->method('deleteKey')
			->with('findling', $this->key());

		$migration = new Version001300Date20260924000000($appConfig);
	...
	public function testTheDroppedVersionIsNamedInTheOutput(): void {
		...
		$output->expects(self::once())
			->method('info')
			->with(self::stringContains('1.2.0'));
	...
	public function testASecondRunDropsNothingAndThrowsNothing(): void {
		...
		// branch and say so.
		$appConfig = $this->createMock(IAppConfig::class);
		$appConfig->method('getValueString')->willReturnOnConsecutiveCalls('1.2.0', '');
